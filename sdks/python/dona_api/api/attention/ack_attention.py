...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> AttentionItem | Error | None:
...
def sync_detailed(
    ...
    *,
    ...
    """Acknowledge
     ...
     Idempotent by nature (no `Idempotency-Key`). Ack never clears a condition; only `ip_blocked` and
    `api_anomaly` resolve on ack (`resolved_reason=acked`). Counts against the writes/min bucket; not
