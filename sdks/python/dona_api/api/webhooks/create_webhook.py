...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 201:
        response_201 = WebhookWithSecret.from_dict(response.json())
...
def sync_detailed(
    *,
    ...
    """Register an endpoint (secret shown once)
     ...
     Dona sends a signed `ping` immediately; your endpoint must answer 2xx within 1 s or the create is
    `422 webhook_url_invalid`. ≤ 5 endpoints per shop: a 6th is `422 webhook_url_invalid` with
    `details[{field:"url",code:"endpoint_limit"}]`. Kill switch: `webhooks_enabled`.
