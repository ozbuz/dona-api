...
class WebhookUpdate implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setUrl($url)
    {
        ...
        if ((mb_strlen($url) > 2048)) {
            throw new \InvalidArgumentException('invalid length for $url when calling WebhookUpdate., must be smaller than or equal to 2048.');
        }
     ...
     * Sets event_types
     *
     * @param string[]|null $event_types Exact event names or prefixes (`order.*`); `[\"*\"]` (or omitted on create) = every event.
     *
     * @return self
     */
    public function setEventTypes($event_types)
    {
        ...
        if ((count($event_types) < 1)) {
            throw new \InvalidArgumentException('invalid length for $event_types when calling WebhookUpdate., number of items must be greater than or equal to 1.');
        }
     ...
     * Sets status
     *
     * @param string|null $status `paused` = seller-side pause (deliveries accrue 30 d); `active` resumes a paused endpoint. A `disabled` endpoint is re-enabled only by a successful `/ping`. Known values (open set — tolerate new ones): `active`, `paused`.
     *
     * @return self
     */
